## Default Permission

Default inspector read/export commands (open, close, getState, exportContext)

#### This default permission set includes the following:

- `allow-open`
- `allow-close`
- `allow-get_state`
- `allow-export_context`
- `allow-copy_context_bundle`
- `allow-copy_screenshot_path`
- `allow-open_screenshot_folder`

## Permission Table

<table>
<tr>
<th>Identifier</th>
<th>Description</th>
</tr>


<tr>
<td>

`visual-editor:all`

</td>
<td>

Full visual editor plugin access

</td>
</tr>

<tr>
<td>

`visual-editor:allow-capture`

</td>
<td>

Enables the capture command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-capture`

</td>
<td>

Denies the capture command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-clear-session`

</td>
<td>

Enables the clear_session command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-clear-session`

</td>
<td>

Denies the clear_session command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-close`

</td>
<td>

Enables the close command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-close`

</td>
<td>

Denies the close command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-copy-context-bundle`

</td>
<td>

Enables the copy_context_bundle command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-copy-context-bundle`

</td>
<td>

Denies the copy_context_bundle command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-copy-screenshot-image`

</td>
<td>

Enables the copy_screenshot_image command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-copy-screenshot-image`

</td>
<td>

Denies the copy_screenshot_image command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-copy-screenshot-path`

</td>
<td>

Enables the copy_screenshot_path command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-copy-screenshot-path`

</td>
<td>

Denies the copy_screenshot_path command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-disable`

</td>
<td>

Enables the disable command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-disable`

</td>
<td>

Denies the disable command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-emit-state`

</td>
<td>

Enables the emit_state command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-emit-state`

</td>
<td>

Denies the emit_state command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-enable`

</td>
<td>

Enables the enable command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-enable`

</td>
<td>

Denies the enable command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-export-context`

</td>
<td>

Enables the export_context command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-export-context`

</td>
<td>

Denies the export_context command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-get-state`

</td>
<td>

Enables the get_state command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-get-state`

</td>
<td>

Denies the get_state command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-hard-reload`

</td>
<td>

Enables the hard_reload command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-hard-reload`

</td>
<td>

Denies the hard_reload command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-open`

</td>
<td>

Enables the open command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-open`

</td>
<td>

Denies the open command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-open-screenshot-folder`

</td>
<td>

Enables the open_screenshot_folder command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-open-screenshot-folder`

</td>
<td>

Denies the open_screenshot_folder command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-pin-target-webview`

</td>
<td>

Enables the pin_target_webview command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-pin-target-webview`

</td>
<td>

Denies the pin_target_webview command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-report-selection`

</td>
<td>

Enables the report_selection command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-report-selection`

</td>
<td>

Denies the report_selection command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-revalidate`

</td>
<td>

Enables the revalidate command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-revalidate`

</td>
<td>

Denies the revalidate command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-set-capture-included`

</td>
<td>

Enables the set_capture_included command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-set-capture-included`

</td>
<td>

Denies the set_capture_included command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-set-issue-text`

</td>
<td>

Enables the set_issue_text command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-set-issue-text`

</td>
<td>

Denies the set_issue_text command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-set-primary-capture`

</td>
<td>

Enables the set_primary_capture command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-set-primary-capture`

</td>
<td>

Denies the set_primary_capture command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-set-target-webview`

</td>
<td>

Enables the set_target_webview command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-set-target-webview`

</td>
<td>

Denies the set_target_webview command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-toggle`

</td>
<td>

Enables the toggle command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-toggle`

</td>
<td>

Denies the toggle command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:allow-unpin-target-webview`

</td>
<td>

Enables the unpin_target_webview command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:deny-unpin-target-webview`

</td>
<td>

Denies the unpin_target_webview command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`visual-editor:capture`

</td>
<td>

Screenshot capture command

</td>
</tr>

<tr>
<td>

`visual-editor:inspect`

</td>
<td>

Selection and session control commands

</td>
</tr>
</table>
